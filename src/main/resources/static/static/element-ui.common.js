...
exports.default = {
  ...
  methods: {
    ...
    handleClick: function handleClick() {
      ...
      let isUseKey = this.tree.props.isFalse?this.tree.props.isFalse:"isFalse";
      if(!this.node.data[isUseKey]){//->如果不是虚机构->改源码判断
      ...
      if (this.tree.expandOnClickNode) {
        ...
        /*************此处是改源码 */
        let valKey = this.tree.props.value?this.tree.props.value:"id";
        if(this.clickTimer)
            window.clearTimeout(this.clickTimer);
        if(this.twiceId){
            if(this.node.data[valKey] === this.twiceId) {
                this.clickCount++;
            }else{
                this.clickCount=1;
                this.twiceId = this.node.data[valKey];
            }
        }else{
            this.clickCount=1;
            this.twiceId = this.node.data[valKey];
