...
public class SheetCategoryController extends BaseEntityController<SheetCategory> {
    ...
    public ResponseResult create(SheetCategory entity) {
        try {
        	...
        	Collection<String> organizationIds = systemUserService.getTreeOrganizationIds(this.getCurrentLoggedInUser().getId(), treeName);
    ...
    public ResponseResult checkAddTaoBiao(String categoryId,Integer param){
    	try {
			if(sysUserService.getAllApprovedEnabledRoleIds(getCurrentLoggedInUser().getId()).contains(0)){
				...
				Collection<String> collection = systemUserService.getTreeOrganizationIds(getCurrentLoggedInUser().getId(), treeName);
