...
import com.sinosoft.ops.cimp.common.word.base.IntervalImpl;
import com.sinosoft.ops.cimp.common.word.model.SetList;
...
public class ResumeAttrValue extends ResumeAttrVauleHandle implements AttrValue {
    ...
    public Object getAttrValue(Map<String, Object> attrValueContext, String empId, ExportService exportWordService) throws Exception {
        ...
        //个人简历子集
        final String resumeInfoSql = "SELECT * FROM EMP_A070 EMP_A070 WHERE EMP_ID = '%s' ORDER BY EMP_A070.A070001 ASC";
        // 学习（培训、进修）情况
        final String trainInfoSql = "SELECT * FROM Emp_A155 Emp_A155 WHERE EMP_ID = '%s' ORDER BY Emp_A155.A155005 DESC";
        //参加重要工作
        final String importantActiveInfoSql = "SELECT * FROM EMP_A145 EMP_A145 WHERE EMP_ID = '%s' ORDER BY A145005 ASC";
        //任职情况
        final String jobInfoSql = "SELECT * FROM Emp_A010 Emp_A010 WHERE EMP_ID = '%s' ORDER BY Emp_A010.A010005,Emp_A010.A010125";
        //教育信息
        final String eduInfoSql = "SELECT * FROM Emp_A005 Emp_A005 WHERE EMP_ID = '%s' ORDER BY Emp_A005.A005015 ASC";
        ...
        if (trainInfoList == null) {
            attrValueContext.put("A155", new ArrayList<Map>());
...
//                        if (StringUtils.equals(a02019, "2")) {
                        for (Object o : A105List) {
                            ...
                            if (A105010 != null) {
                                DateTime dateTime = new DateTime(A105010);
...
//                stringBuffer.append(a08016);
                ...
                DateTime dateTime = new DateTime(df.parse(A005015));
...
//                temp.append(isRealse);
                ...
                if (interval.contains(dateTime) && interval.contains(new DateTime(StringUtils.isEmpty(A005035) ? new DateTime() : df.parse(A005035)))) {
                    strList.add(temp);
                } else if (!interval.contains(dateTime) && interval.contains(new DateTime(StringUtils.isEmpty(A005035) ? new DateTime() : df.parse(A005035)))) {
                    corssList.add(temp);
                }
            }
        }
        ...
        for (Object importantActiveInfo : importantActiveInfoList) {
            ...
            if (StringUtils.isNotEmpty(StringUtil.obj2Str(importantActiveStartDate))) {
                DateTime startTime = new DateTime(df.parse(importantActiveStartDate.toString()));
                String startTimeStr = startTime.toString("yyyy.MM");
                if (StringUtils.isNotEmpty(StringUtil.obj2Str(importantActiveEndDate))) {
                    DateTime endDateTime = new DateTime(importantActiveEndDate);
                    ...
                    } else if (interval.contains(endDateTime) && !interval.contains(startTime)) { //结束日期在时间断内，开始日期不在
            ...
            if (StringUtils.isNotEmpty(trainStartDate)) {
                ...
                if (trainDays >= 60) {
                    if (StringUtils.isNotEmpty(trainEndDate)) {
                        ...
                        } else if (interval.contains(trainEndDateTime) && !interval.contains(dateTime)) { //结束日期在时间断内，开始日期不在
        ...
        for (Object jobInfo : jobInfoList) {
            ...
            if (StringUtils.isNotEmpty(StringUtil.obj2Str(jobStartDate))) {
                DateTime startTime = new DateTime(df.parse(jobStartDate.toString()));
                String startTimeStr = startTime.toString("yyyy.MM");
        ...
        if (strList.size() > 0 && corssList.size() > 0) {
            betweenStringBuilder.append("（").append(org.apache.commons.lang3.StringUtils.join(strList, ";"));
