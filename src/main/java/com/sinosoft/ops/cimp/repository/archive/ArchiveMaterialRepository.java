...
public interface ArchiveMaterialRepository extends JpaRepository<ArchiveMaterial,String>, QuerydslPredicateExecutor<ArchiveMaterial> {
        ...
        @Query("select a from ArchiveMaterial a where empId=?1 and a.categoryId=?2 order by a.seqNum")
        ...
        @Query(value = "SELECT  EMP_ID FROM  SYS_USER  WHERE ID=?1 AND STATUS=0",nativeQuery=true)
