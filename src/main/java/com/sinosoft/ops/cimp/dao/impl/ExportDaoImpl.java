...
public class ExportDaoImpl extends BaseDaoImpl implements ExportDao {
  ...
  /*  	if(sessionFactory==null){
    		...
    		ApplicationContext ct=new FileSystemXmlApplicationContext(path+"//src//spring//applicationContext.xml");
    		sessionFactory=(SessionFactory) ct.getBean("sessionFactory");
    	}*/
    	List<Map<String, Object>> list =  sessionFactory.getCurrentSession().createSQLQuery(sql).setResultTransformer(CriteriaSpecification.ALIAS_TO_ENTITY_MAP).list();
    	return list;
    }
  ...
  /*  	if(sessionFactory==null){
    ...
    public String getEmpsInUnitExportGbrmb(String[] depIds) {
        ...
        List list = sessionFactory.getCurrentSession().createSQLQuery(sql).setParameterList("depIds", depIds).list();
    ...
    public String getStringEmps(String type) {
    	...
    	List list=null;
    	if(flag){
    		list = sessionFactory.getCurrentSession().createSQLQuery(sb.toString()).setParameter("treeLevelCode",type+'%').list();
    	}else{
...
//                    new ArrayBlockingQueue<Runnable>(12000), threadFactory);
//            //守护线程
